...
def bsp_findings(response: str, index: BspIndex | None,
    ...
    Без этого вызов метода на переменной читается как обращение к общему
    модулю: ДвоичныеДанные.Размер() на локальной переменной по форме
    неотличим от вызова экспортного метода.
