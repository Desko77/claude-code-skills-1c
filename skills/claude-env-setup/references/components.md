...
сравнение содержимого с игнором EOL (`diff -w --strip-trailing-cr`). Для `agents/` есть штатная
проверка установщика: `python tools/install_home.py --check` (0 - совпадает).
...
исполняемым установщиком из корня репозитория:
