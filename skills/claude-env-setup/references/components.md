...
**Что.** Содержимое этого репозитория: `skills/`, `rules/`, `commands/`, `scripts/`, `agents/`.
...
сравнение содержимого с игнором EOL (`diff -w --strip-trailing-cr`). Для компонентов установщика
(таблица ниже) есть штатная проверка: `python tools/install_home.py --check` (0 - совпадает).
...
| Компонент | Что ставится | Адресат |
|---|---|---|
...
копией в `~/.claude/backup/install-<время>/`.
...
**Что.** Публичный MCP-сервис `https://ai.v8std.ru/mcp`: текст стандарта 1С по номеру (`stdNNN`),
расшифровка диагностик ACC/BSLLS/EDT v8-code-style, поиск по теме, чтение страницы по id или URL.
...
**Локальный вариант.** Необязательный свой экземпляр в Docker (`http://127.0.0.1:18766/mcp`) - для
обращений с фрагментами кода; публичному сервису передавать только номера стандартов, коды
диагностик и обезличенные фрагменты, закрытый код не передавать.

**Проверка.** В `tools/list` сессии присутствуют инструменты `v8std_*` (поиск, чтение страницы,
расшифровка диагностик).
