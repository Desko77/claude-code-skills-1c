...
Агенты набора (`agents/`) ставятся не копированием, а исполняемым установщиком из корня
репозитория - он ведет манифест установленных файлов и не затирает измененное снаружи:

```bash
python tools/install_home.py install --components agents   # установка
