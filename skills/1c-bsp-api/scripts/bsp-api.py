...
def main():
    ...
    if args.command == 'check':
        ...
        if hit:
            ...
            # Существует - еще не значит "вызывай". Устаревший метод по виду не
            # отличается от действующего, и проверка одного лишь существования
            # пропускает его молча: на этом набор уже обжегся, заведя кейс на
            # ДлительныеОперации.ВыполнитьВФоне. Код возврата не меняем: метод
            # существует, а предупреждение читает человек либо валидатор.
            dep = hit[0].get('dep')
            if dep:
