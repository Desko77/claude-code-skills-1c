...
| любой "not available" в начале сессии | сервер `1c-edt` не отвечает - причина пока неизвестна | `get_edt_version` как проба. Ошибка связи или timeout - `self_status` НЕ вызывать (он на том же сервере), диагноз "1c-edt недоступен". Сервер ответил, но операция не прошла - уточнить через `self_status`. Разбор случаев и что при этом можно продолжать - `rules/mcp-tool-priority.md`, раздел "Когда инструменты недоступны" |
...
3. Профильный фасад: `project_admin`, `infobase_admin`, `config_io`, `diagnostics`, `insights`,
   `security_audit`, `data_access`, `docs_lookup`, соответствующий `*_workshop`.
...
## Экспериментальные и ненадежные инструменты

`browse_data` и `execute_query` помечены в форке как experimental / non-functional. Фасад `data_access` их
только проксирует и работает лишь из приостановленной debug-сессии - неработоспособность он не устраняет.

Однозначный маршрут к данным живой базы: **вне отладки - скил `1c-mcp-toolkit` (HTTP API к
запущенной 1С); внутри приостановленной debug-сессии допустимо попробовать `data_access`, и при отказе не
чинить его, а уходить на HTTP-маршрут.**
...
выше), а найденный дефект плагина фиксировать в трекере обратной связи по плагину.
