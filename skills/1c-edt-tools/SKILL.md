...
MCP-сервер **1c-edt** дает прямой доступ к семантическому индексу EDT (BM model), платформенной
...
`get_edt_version` - проба. Не ответил - корректный вывод "1c-edt недоступен", а НЕ "EDT не запущена": та же
...
| `data_access` | ТОЛЬКО при приостановленной debug-сессии, и объединяемые им `browse_data` / `execute_query` помечены в форке experimental. Для данных живой базы предпочесть скил `1c-mcp-toolkit` |
...
Каталог в `references/` - снимок docs форка EDT-MCP на ревизии `4fb31770` (28.07.2026), 118 имен сверены с
...
инструментов. Каталог устарел - пересобрать снимок из docs форка, а не дописывать по памяти: именно так в
