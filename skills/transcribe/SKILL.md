...
argument-hint: "<FilePath> [--output-dir DIR] [--engine local|gemini] [--analyze-ui] [--with-summary] [--diarize] [--num-speakers N] [--diarize-engine sherpa-onnx|pyannote|moss] [--project NAME] [--no-voiceprints] [--no-coherent]"
allowed-tools:
  - Bash
  - Read
  - Glob
