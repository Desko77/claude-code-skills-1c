...
- `<номер>` - 6 цифр монотонной последовательности отметки времени. Номер
  ...
  повторяет ту же схему резервирования;
...
пройдено: любой `applied` с `critical` больше 0 в прогоне проверяется раньше прочих
событий той же проверки и блокирует вердикт до снятия - пропуск с классом либо другое
`applied` без critical его не перекрывают.
