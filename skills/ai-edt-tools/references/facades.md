...
Используйте фасад для исследования BSL и модели перед изменениями. `describe_db_tables` (доступен и отдельным именем) показывает, во что объект превращается в базе
...
- **`import_configuration_from_binary`** принимает `.cf` или `.cfe` одним файлом и разворачивает его в
  ...
  или `1c-db-load-cf` из набора, они работают без EDT.
