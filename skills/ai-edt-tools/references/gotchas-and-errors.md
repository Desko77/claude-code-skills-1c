...
## Тяжелые вызовы и формат файла сведений выгрузки (0.2.55)

Тяжелые вызовы - обновление базы, в том числе перед запуском клиента (`updateBeforeLaunch`), выгрузки
и загрузки конфигурации, запуск Конфигуратора, создание базы, широкие обходы проекта - выполняются не
больше трех одновременно; при остатке кучи EDT выше 92 % после сборки мусора новый тяжелый вызов
отклоняется до начала работы (`503` с `Retry-After`). Отказ по пределу - повторить позже, а не
параллелить. Ответ `Pending` держит разрешение до конца фоновой работы; опрос по `runKey` разрешения
не занимает.
