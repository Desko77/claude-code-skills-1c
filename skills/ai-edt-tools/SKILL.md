...
конкретной операции - `operation=help topic=<операция>`. `find=<слова>` ищет по справке фасада и
называет тему, по которой открыть раздел целиком.
...
| `git` | репозиторий проекта внутри EDT: статус, ветки, история, коммит названных файлов, переход по веткам (записи гасятся как `git_commit` / `git_checkout`) |
