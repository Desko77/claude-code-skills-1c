...
`НачатьТранзакцию()` ставится строго до `Попытка`, и между ними нет другого кода.

Канонический шаблон целиком, неявные транзакции платформы, управляемые блокировки и порядок захвата -
...
```bsl
// Плохо: тело цикла обернуто в условие
Для Каждого Строка Из ТаблицаДанных Цикл
...
| Анти-паттерн | Серьезность | Что искать |
|-------------|-------------|------------|
