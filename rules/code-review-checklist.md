...
Часть проверок можно автоматизировать при наличии MCP-серверов:

| Что проверить | Инструмент (сервер) |
|---------------|---------------------|
| Ошибки проекта (синтаксис, типы, неиспользуемые переменные) | `get_project_errors` (1c-edt) |
| Синтаксис BSL (без EDT) | `check_syntax` / `get_diagnostics` (1c-syntax-checker-mcp) |
| Стиль и архитектура кода | `check_code` / `analyze_module` (1c-code-check-mcp) |
| Корректность запросов | `validate_query` (1c-edt) |
| Описание конкретной ошибки EDT | `get_check_description` (1c-edt) |
| Перевалидация после исправлений | `revalidate_objects` (1c-edt) |
