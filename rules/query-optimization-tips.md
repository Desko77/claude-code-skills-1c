...
Анти-паттерны производительности - карточки `PERF-01`, `PERF-02`, `PERF-03` каталога дефектов (скил `1c-code-review`); советы этого файла перенесены в карточки `QUERY-04` - `QUERY-12`. Продвинутые паттерны (временные таблицы, пакетные запросы, СКД) - см. скил `/1c-query-optimization`.
...
- `validate_query` (ai-edt) - проверка синтаксиса и семантики запроса в контексте проекта
