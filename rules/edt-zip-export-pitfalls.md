...
В типовой конфигурации (без расширения) у части подсистем `.cmi` отсутствует и все работает - там
...
- [ ] `<name>` уникальное (с префиксом расширения, напр. `Ext_*`)
...
- [ ] Реквизиты типа `DocumentRef.X` / `CatalogRef.X` и составные: `add_object_attribute` их **персистит** - main config И adopted-объект расширения (проверено 2026-06-12 на демо-БСП: `.mdo` расширения получает `<types>CatalogRef.X</types>`). Ручная правка `.mdo` для СОЗДАНИЯ реквизита больше не нужна. Сменить тип УЖЕ существующего реквизита на составной - `set_object_type ownerFqn=<owner>.Attribute.<имя> type="CatalogRef.A,DocumentRef.B"` (child-FQN, 1.43.x; на старой сборке без фикса - временно `.mdo`). (прежнее утверждение "не персистит" устарело, пересмотрено 2026-06-12)

---

## 2а. Профилактика: сбросить модель на диск перед выгрузкой

Ошибка `zip:///` возникает потому, что EDT держит объект в памяти, а на диске лежит устаревшее или
неполное представление - платформа читает диск и не находит того, что EDT считает существующим.
Лечится это до того, как случится: **`project_admin operation=resync_to_disk`** форсирует выгрузку
модели указанных объектов из памяти на диск. Плагин прямо называет назначение инструмента: звать
перед `update_database` и перед экспортом конфигурации, чтобы не получить класс ошибок
"валидно в EDT, падает в ИБ" и `zip:///` not-found.

Что он НЕ делает: не вычищает ссылки из `Configuration.mdo`. Если объект зарегистрирован в составе
конфигурации, а самого объекта нет, сброс на диск это не исправит - это другой дефект.
...
5. **Если файл есть, но поврежден** - сравнить с эталоном (типовая конфигурация рядом).
...
## 5. После обновления из хранилища 1С (импорт в EDT)

Обратное направление (хранилище/Конфигуратор -> EDT) дает свои симптомы:

- **Ложные массовые diff ("все файлы изменены", расхождения в отступах/EOL).** Перед разбором изменений
  прогнать нормализацию концов строк согласно `.gitattributes` (`git add --renormalize .` при необходимости)
  и сверять diff с игнором пробелов (`git diff -w`). Детали EOL-политики - в правилах работы с git проекта (`.gitattributes`).
- **Форма открывается в EDT, но битая в Конфигураторе после импорта.** Полная выгрузка/загрузка НЕ лечит.
  Рабочий рецепт - пересоздание элемента: удалить проблемный реквизит формы и создать заново с теми же
  свойствами (перепривязать элементы формы).
- **Правило: источник истины - EDT.** После любой правки в Конфигураторе - обязательный реимпорт в EDT
  до продолжения работ, иначе следующий экспорт из EDT затрет правки.
