...
- **session-context** (`SessionStart`): сообщает модели идентификатор сессии следа
  ...
  `--session`) и вычищает каталоги сессий старше 7 дней, а также каталог ключа без сессий,
если каждый файл в нем старше того же срока. Работает одинаково при старте,
...
База - `QUALITY_STATE_DIR`, если переменная не пустая, иначе
