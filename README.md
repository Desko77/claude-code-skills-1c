...
Требуется Node.js 18+. Текущее состояние: **536 зеленых из 822**, 66 пропущено по условиям среды.
Остальные 220 кейсов описывают функциональность, которой в скриптах пока нет: тип объекта, операция
или параметр из исходного набора, до которых очередь не дошла. Прогон падает только на неожиданном
отказе. Порядок работы с тестами - `tests/skills/README.md`.
