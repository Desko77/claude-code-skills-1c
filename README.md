...
Полный перечень с описанием каждого скила - в разделе [Скилы](#скилы-117) ниже.
...
Плюс **1 агент** (`agents/`) - субагент-разведчик 1С с ограничением инструментов, ставится в
`~/.claude/agents/` установщиком набора (раздел [Агенты](#агенты-1) ниже).
...
| `anti_patterns.md` | Таблица триггеров каталога дефектов (генерируется из карточек): производительность, запросы, транзакции, клиент-серверные вызовы |
...
| `code-review-checklist.md` | Чеклист ревью BSL-кода: классификация находок, шкала уверенности, security-практики, сгенерированная секция каталога дефектов |
...
Субагенты Claude Code из каталога `agents/` - изолированные исполнители с ограниченным набором
инструментов. Список `tools` у агентов набора не задается: закрытый список не наследует
MCP-серверы вызывающей сессии, ограничение задается через `disallowedTools`.

| Агент | Что делает | Как ограничен | Установка |
|-------|------------|---------------|-----------|
| `1c-explore` | Разведка EDT-проекта 1С: кто вызывает метод, структура модуля, состав метаданных объекта, трассировка "как работает X"; отчет по формату `rules/code-exploration-guide.md` | `disallowedTools: Grep, Glob, Bash, PowerShell` - ответы только через семантический индекс AI-EDT, без перебора исходников; при недоступном AI-EDT останавливается с отчетом первой строкой, решение о переборе принимает вызывающая сессия | `python tools/install_home.py install --components agents` |

Установщик ведет манифест установленных файлов (`~/.claude/.install-manifest.json`, sha256),
измененный снаружи файл не затирает (конфликт, код выхода 1), по `--force` переписывает
с резервной копией; `--check` сверяет домашние копии с репозиторием.

Контур качества в `tools/`:

| Инструмент | Что делает |
|------------|------------|
| `changeset.py` | Каноническое множество изменений git-репозитория относительно базового коммита: записи путей со статусами и хешами, `diffHash` |
| `change_profile.py` | Профиль правки по множеству изменений: класс объема C0-C3, архетипы, среда, обязательный состав проверок; пишет событие scope в след (`skills/1c-code-review/references/profile-map.md`) |
| `evidence.py` | Каталог событий следа проверок: запись `skipped`/`not_verified`/`probe`, строгий вердикт `check --strict` (clean / with_gaps / blocked), отчет `render` (`skills/1c-code-review/references/evidence-format.md`) |
...
| `1c-defect-catalog-spec.md` | Спецификация каталога дефектов: карточки, реестр идентификаторов, словарь детекторов с уровнями доказательности, фикстуры, генерация индекса |
...
В каталоге `hooks/` девять хуков Claude Code, дополняющих проверки, встроенные в сами скилы:
...
- **skill-suggester** - при ручной работе с исходниками 1С напоминает про профильный скил: при
  ...
  сессию на группу.
- **evidence-writer** - после вызова инструмента проверки (MCP AI-EDT, напарник или скрипт набора)
  записывает событие `applied` с итогом в след проверок, при отказе инструмента - `failed`;
  событие пишет хук по факту вызова, из CLI его не набрать.
- **session-context** - при старте сессии сообщает ее идентификатор для следа проверок (довод
...
- **quality-baseline** - при старте сессии запоминает `HEAD` и хеши файлов, чтобы отличать правки
  этой сессии от чужих; при `resume`, `compact` и `clear` отметку не переписывает.
- **quality-arm** - отмечает, каким инструментом правился файл 1С (`Write`, `Edit`, MCP-правки
  AI-EDT).
...
Идеи раздела "Сначала индекс" в правиле маршрутизации MCP, каталога дефектов и следа проверок -
из репозитория [Romandredan/1c-quality-gate](https://github.com/Romandredan/1c-quality-gate)
(MIT); код и тексты не переносились.
