...
Отдельный класс - **справочники API**. Агент не угадывает, какой метод вызвать, а берет его из
справочника и может проверить вызов до того, как его напишет. `1c-bsp-api` покрывает Библиотеку
стандартных подсистем: 2620 методов в 283 модулях, с сигнатурами, типами параметров, контекстами
выполнения и переопределяемыми обработчиками. `zup-hr-api-reference` - 1С:ЗУП 3.1: кадровый учет,
средний заработок и начисления, пособия и обмен с СФР, регламентированная отчетность.
...
| Справочные и утилитарные | документация платформы, справочники API типовых, транскрибация, docx и xlsx, окружение агента | 28 |
